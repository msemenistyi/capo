...
!(function(window, document){
	...
	function EventItem(name){
		...
		this.triggers = [];
		this.$el = {
			strangeEventsContainer: document.getElementById('strange-events-container-itself'),
			container: document.getElementById('events-container-itself'),
	...
	EventItem.prototype.renderToList = function(stage) {
		...
		if (stage ==='initial' && (!this.subscriptions.length || !this.triggers.length)){
			var copyEl = el.cloneNode();
			copyEl.innerHTML = content;
			this.$el.strangeEventsContainer.appendChild(copyEl); 
		}
	};
	...
	EventCollection.prototype.renderEvents = function(eventList, stage) {
		this.clearContainer();
		if (!eventList){
			for (i = 0, l = this.events.length; i < l; i++){
				this.events[i].renderToList(stage);
			}		
		} else {
			for (i in eventList){
				eventList[i].renderToList(stage);
			}			
		}
		if (this.$el.strangeEventsContainer.textContent === ''){
			this.$el.strangeEventsContainer.parentNode.textContent = '';
		}
	};
	...
	function App(){
		...
		this.$el = {
			...
			secArrowDown: document.getElementById('sec-arrow-down'),
			secArrowUp: document.getElementById('sec-arrow-up'),
			themeSwitcher: document.getElementById('theme-switcher')
		};
		this.eventCollection = new EventCollection();
		this.eventCollection.populateEvents();
		this.eventCollection.renderEvents(false, 'initial');
		this.gatherCodeBlocks();
		prettyPrint(function(){
			this.activateLinesForCodeBlocks();
			this.selectedEventContainer = undefined;
			this.areStrangeEventsShown = false;
		}.bind(this));
	}
	...
	App.prototype.bindListeners = function() {
		...
		this.$el.eventsWrapper.addEventListener('click', function(e){
			...
			if (eventContainer){
				this.filterCodeBlocks(eventContainer.querySelector('.event-name').textContent);
		...
		this.$el.secLabel.addEventListener('click', function(e){
			this.areStrangeEventsShown = !this.areStrangeEventsShown;
			if (this.areStrangeEventsShown){
				this.$el.secArrowUp.style.display = 'block';
				this.$el.secArrowDown.style.display = 'none';
				this.$el.strangeEventsContainer.style.display = 'block';
			} else {
				this.$el.secArrowUp.style.display = 'none';
				this.$el.secArrowDown.style.display = 'block';				
				this.$el.strangeEventsContainer.style.display = 'none';
			}
		}.bind(this));

		this.$el.themeSwitcher.addEventListener('click', function(e){
			if (this.dayTheme){
	...
	App.prototype.activateLinesForCodeBlocks = function() {
		...
		for (var i = 0, l = els.length; i < l; i++){
			...
			activeLineNumber = els[i].querySelector('.cb-line-number').textContent;
